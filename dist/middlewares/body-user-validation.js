...
exports.bodyUserValidation = {
    ...
    newPassword: (0, express_validator_1.body)('newPassword').isString().notEmpty().trim().isLength({ min: 6, max: 20 }),
    email: (0, express_validator_1.body)('email').isString().notEmpty().trim().matches('^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$'),
    recoveryCode: (0, express_validator_1.body)('recoveryCode').isString().notEmpty().trim()
};
